...
# AGRUPACION - se usa para agurpar expresiones, para ello se utilizan los parentesis '()'. esta heramienta es muy potente, ya que despues se puede reemplazar todo lo que no este en grupos, pornada y dejar unicamente lo que estaba dentro de grupos con la notacion '$1,$2,...' y asi sucesivamente, dependiendo la cantidad de grupos que hayamos declarado. posteriormente esto se  puede llevar a SQL o JSON
...
# REGEX EN PYTHON - https://docs.python.org/3/library/re.html#functions (OFFICIAL DOCUMENTATION)
    ...
    # 1. se debe importar la librería re
         # import re
    # se usa (r'regex') => regex es la expresion regular
...
"""
    ...
    # x = re.sub("\s", "9", txt)
    #            ___   ___  ____
    #            1     2     3  => 1. regex // 2. texto por el que queremos reemplazar la coincidencia // 3. string en el que vamos a evaluar el  regex y hacer los cambios.
    #Example:
"""
        txt = "The rain in Spain"
        x = re.sub("\s", "9", txt)
